...
function Result(props) {
    ...
    return (
        <div className="result">
            <h2 className="text-center font-weight-normal">پاسخ ها</h2>
            {questions.map((q, index) =>
                <div key={q.id} className={`mb-2 ${q.isCorrect ? 'bg-success' : 'bg-danger'}`}>
                    <div className="result-question">
                        ...
                        <div className={`m-1 p-1 text-bold ${q.isCorrect ? 'text-success' : 'text-danger'}`}>جواب شما به سوال {q.isCorrect ? 'صحیح' : 'اشتباه'} بوده است.</div>
                    </div>
                </div>
            )}
            <h4 className="alert alert-info text-center">پایان</h4>
        </div>
    )
}
